...
npm list electron electron-builder >nul 2>&1
if %errorlevel% neq 0 (
    echo electron electron-builder module is not installed. Installing them...
    npm install electron@34.3.0 electron-builder@24.2.0
    if %errorlevel% equ 0 (
        echo Installation of electron electron-builder completed successfully !
    ) else (
        echo An error occurred while installing the electron electron-builder modules.
    )
) else (
